/**
 * SUBMODAL v1.6 Used for displaying DHTML only popups instead of using buggy
 * modal windows.
 * 
...
// var gDefaultPage = "../../commons/templates/loading.html";
...
function initPopUp() {
	...
	popcont.innerHTML = ''
			...
			+ '<img src="' + contextPath + '/img/ico_close.png" onclick="hidePopWin(false);" id="popCloseBox" />'
			+ '</div>'
			+ '</div>'
	...
	for (i = 0; i < elms.length; i++) {
		if (elms[i].className.indexOf("submodal") == 0) {
			...
			// (){showPopWin(\''+elms[i].href+'\','+width+', '+height+',
			// null);return false;};';
...
function showPopWin(url, width, height, returnFunc, title) {
	...
	var titleBarHeight = parseInt(
			document.getElementById("popupTitleBar").offsetHeight, 10);
	...
	// border in IE6
	gPopFrame.style.width = parseInt(
...
function centerPopWin(width, height) {
	if (gPopupIsShown == true) {
		...
		gPopupContainer.style.top = (scTop + ((fullHeight - (height + titleBarHeight)) / 2))
				+ "px";
		gPopupContainer.style.left = (scLeft + ((fullWidth - width) / 2))
				+ "px";
...
function setMaskSize() {
	...
	gPopupMask.style.height = popHeight + 300 + "px";
	gPopupMask.style.width = popWidth + "px";
}

/**
 * @argument callReturnFunc - bool - determines if we call the return function
 *           specified
 ...
 * 
 * IE has a problem with wanting select form tags to always be the topmost
 * z-index or layer.
 */
